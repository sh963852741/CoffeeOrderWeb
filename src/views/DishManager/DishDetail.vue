        ...
        <Row type="flex">
            ...
            <i-col span="10">
                <Form :model="mealInfo" label-position="left" :label-width="80">
                    ...
                    <FormItem label="餐点图片">
                        <img
                            :src="`/CoffeeOrderService/api/menu/downloadImg?mealId=${mealInfo.mealId}`"
                            alt="暂无菜品图片"
                            style="width:100%"
                        />
                    </FormItem>
                    <FormItem>
                        <Upload
                            :action="`/CoffeeOrderService/api/menu/uploadImg?mealId=${mealInfo.mealId}`"
                            :format="['jpg','jpeg','png']"
                            :on-success="getImg"
                            :max-size="1024"
                        ...
                        <Button
                            type="primary"
                            style="float:right;margin:0px 0px 30px 0px;"
                            @click="saveMealInfo"
                        >
                            <Icon type="md-copy" />保存
                        </Button>
...
export default {
    ...
    methods: {
        ...
        getMealDetail() {
            axios
                .post("/CoffeeOrderService/api/menu/getMeal", {
                    mealId: this.mealInfo.mealId
                })
                .then(response => {
        ...
        saveMealInfo() {
            axios
                .post("/CoffeeOrderService/api/menu/modifyMeal", {
                    mealId: this.mealInfo.mealId,
